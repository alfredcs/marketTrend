...
def modify_date(data):
    ...
    dates = data['created_at'] # get the datetime field
...
def main():
    ...
    kinesis = create_client('kinesis','us-west-2')
    
    # get tweets
    now = datetime.now()
    local_filename="twitter_"+str(int(datetime.timestamp(now)))+".csv"
    #key_words="ai OR \\artificial intelligence\\ OR ml OR \\machine learning\\ OR \\deep learning\\ OR cv OR \\computer vision\\ OR NLP OR \\natural language processing\\ OR \\time series\\ OR forecast "
    key_words="arxiv OR learning OR \time series\ OR AI"
    get_tweets(key_words, local_filename, "2020-01-01")

    # load in data from the csv
    col_list = ["conversation_id","created_at","timezone","place","tweet","language"]
    data = load_data(local_filename, col_list)
    ...
    os.remove(local_filename) if os.path.exists(local_filename) else None
